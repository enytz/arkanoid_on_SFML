...
bool Game::Restart()
{
    ...
    ShowResult();
    std::this_thread::sleep_for(std::chrono::milliseconds(200));    // for delay in term isPressButton from ShowResult and new game
    window.BeginDraw();
    sf::Text queryToOut;
...
void Game::SaveResult()
{
    ...
    if (!ifs)
    {
        std::cout<< "Error opening file "<<fileResultScore<<std::endl<<"Create file: "<<fileResultScore<<std::endl;
    }
    else
    {    
    while(!ifs.eof())
    ...
    //scoreResult.push_back(data);
    if (scoreResult.empty())
        {
            ofs<<data.first<<' '<<data.second<<std::endl;
            ofs.close();
            return;
        }
    else if(colResult >= scoreResult[scoreResult.size()-1].second)
    {
        if (scoreResult.size() >= 10)
        {
            scoreResult.erase(scoreResult.end());
        }
...
void Game::ShowResult()
{
    ...
    std::string playerNameScoreResult{}, scoreResult{};
    if (!ifs)
    {
        std::cout<< "Error opening file "<<fileResultScore<<std::endl;
        return;
    ...
    {
        ifs>>data.first>>data.second;
        playerNameScoreResult += data.first+'\n';
        scoreResult += data.second+'\n';       
    }
    ifs.close();
    }
    sf::Text playerName;
    sf::Text playerResult;

    window.BeginDraw();

    SetStringOnDisplay(playerName, font,playerNameScoreResult,150,100);
    SetStringOnDisplay(playerResult,font,scoreResult,400,100); 
    window.Draw(playerName);
    window.Draw(playerResult);
    window.EndDraw();
...
void PrintInputOnScreen(Window& window, sf::Font& font,std::string stringName, std::string& str, float posX, float posY)
{
    ...
    while(!((sf::Keyboard::isKeyPressed(sf::Keyboard::Enter))))
    {
        ...
        window.Draw(strName);
        window.EndDraw();
        if (str.size() >= 15)       // maximum name length
            break;
